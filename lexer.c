...
const char *SYM_NAMES[] = {
    ...
    [SEOF]="SEOF", [SERROR]="SERROR", [MIN]="MIN" , [MAX]="MAX" , [EQL]="EQL" , [MINE]="MINE" , [MAXE]="MAXE",
    [NEQL]="NEQL"
};
...
void next_symbol()
{
    ...
    case '<': 
        if(input[ic] == '='){
            lex_symbol = MINE;   break;
        }else{
            lex_symbol = MIN;   break;
        }
    case '>': 
        if(input[ic] == '='){
            lex_symbol = MAXE;   break;
        }else{
            lex_symbol = MAX;   break;
        }
    case '=': lex_symbol = EQL;   break;
    case '!': 
        if(input[ic] == '='){
            lex_symbol = NEQL;   break;
        }else{
            break;
        }
